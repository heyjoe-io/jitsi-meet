...
import { uploadLocalRecordingNative } from '../functions'
...
import { SET_LOCAL_RECORD_AUTO_UPLOAD, SET_ENABLE_1080P, REMOVE_NATIVE_LOCAL_RECORDING } from '../actionTypes'
    ...
    return (
        <View style={{
            ...
            <View style={{ flex: 1, minWidth: 0 }}>
                <Text style={[style.recordingItemText, {
                    fontSize: 18,
                }]}>
                    {moment(item.timestamp).format('YYYY/MM/DD hh:mm A')}
                </Text>
                <Text style={style.recordingItemText}>{item.fileSize}</Text>
            </View>
            <View style={{flexDirection: 'row', gap: 12 }}>
                ...
                {item.status === 'uploading' && (
                    <View style={{flexDirection: 'row', gap: 8, marginLeft: 8}}>
                        ...
                        <TouchableOpacity onPress={onUpload} style={[style.button, {
                ...
                <TouchableOpacity onPress={onShare} style={style.button}>
                    <View>
                    {Platform.OS === 'android' ? (
...
const LocalRecordingList = ({
    ...
    enable1080p, setEnable1080p, removeLocalRecordings
}) => {
    ...
    return (
        <SafeAreaView style={{ flex : 1 }}>
            <View style={style.optionItem}>
                <Text style={{ color: 'white', fontSize: 16 }}>Auto Upload?</Text>
                <Switch
            ...
            <View style={style.optionItem}>
                <Text style={{ color: 'white', fontSize: 16 }}>Quality</Text>
                <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
                    ...
                    />
                    <Text style={{ color: 'white' }}>1080p</Text>
                </View>
            </View>

            <View style={style.optionItem}>
                <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 18 }}>
                    {state['features/talent'].studio.name} 
                </Text>
                {data.filter(item => item.status === 'pending').length > 0 && (
                    <TouchableOpacity onPress={uploadAll}>
                        <View style={style.uploadButton}>
                            <IconCloudUpload fill={BaseTheme.palette.action01} />
                            <Text style={style.uploadButtonText}>Upload All</Text>
                        </View>
                    </TouchableOpacity>
                )}
            </View>
