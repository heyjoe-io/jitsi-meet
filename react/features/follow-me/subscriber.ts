 ...
 */
StateListenerRegistry.register(
    /* selector */ state => state['features/follow-me'].followMeRecorderEnabled,
    /* listener */ (newSelectedValue, store) => {
        ...
        if (!newSelectedValue && !store.getState()['features/follow-me'].followMeEnabled) {
...
function _getFollowMeState(state: IReduxState) {
    ...
    return {
        recorder: state['features/follow-me'].followMeRecorderEnabled ? 'true' : undefined,
...
function _sendFollowMeCommand(
    ...
    if (newSelectedValue === 'off') {
        ...
        && !state['features/follow-me'].followMeRecorderEnabled) {
        return;
    }
